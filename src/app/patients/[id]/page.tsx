    ...
    <main className="min-h-screen bg-background px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          ...
          <article className="rounded-3xl border border-border bg-surface p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-medium">Histórico do paciente</h2><p className="mt-1 text-sm text-muted-foreground">Consultas e observações em ordem cronológica.</p></div><span className="rounded-full bg-brand-soft px-3 py-1 text-xs font-medium text-brand">{appointments.length + observations.length} registro{appointments.length + observations.length === 1 ? "" : "s"}</span></div>
            {patient.active ? <form onSubmit={addObservation} className="mt-6 rounded-2xl border border-border bg-background p-4"><label htmlFor="patient-observation" className="flex items-center gap-2 text-sm font-medium"><MessageSquareText size={16} className="text-brand" />Adicionar observação</label><p className="mt-1 text-xs text-muted-foreground">Registro administrativo com autor, data e horário. Não substitui o prontuário clínico.</p><textarea id="patient-observation" required maxLength={2000} value={observationText} onChange={(event) => setObservationText(event.target.value)} placeholder="Escreva uma observação sobre este paciente" className="mt-3 min-h-24 w-full resize-y rounded-xl border border-border bg-surface px-3 py-2 text-sm" /><div className="mt-2 flex justify-end"><button type="submit" disabled={savingObservation || !observationText.trim()} className="inline-flex h-10 items-center gap-2 rounded-xl bg-brand px-4 text-sm font-medium text-white hover:bg-brand-hover disabled:opacity-60"><Plus size={15} />{savingObservation ? "Salvando…" : "Registrar observação"}</button></div></form> : null}
