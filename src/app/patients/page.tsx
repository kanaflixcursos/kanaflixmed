...
import { ArrowLeft, CalendarDays, Mail, Phone, Plus, Search, UserRound, X } from "lucide-react";
...
export default function PatientsPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error === "AUTH_REQUIRED" ? "Entre na sua conta antes de cadastrar um paciente." : payload.error === "PATIENT_ALREADY_EXISTS" ? "Já existe um paciente com esses dados." : "Revise os dados e tente novamente.");
      setForm(emptyForm);
    ...
    <main className="min-h-screen bg-background px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 rounded-3xl border border-border bg-surface shadow-sm"><div className="flex flex-col gap-4 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 className="text-lg font-medium">Todos os pacientes</h2><p className="mt-1 text-sm text-muted-foreground">{patients.length} cadastro{patients.length === 1 ? "" : "s"} ativo{patients.length === 1 ? "" : "s"}</p></div><label className="relative block sm:w-80"><Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input className="h-11 w-full rounded-xl border border-border bg-background pl-10 pr-4 text-sm outline-none transition focus:border-brand" placeholder="Buscar por nome" value={search} onChange={(event) => setSearch(event.target.value)} /></label></div>
          ...
          {isLoading ? <div className="p-12 text-center text-sm text-muted-foreground">Carregando pacientes...</div> : filteredPatients.length === 0 ? <div className="flex flex-col items-center px-6 py-16 text-center"><span className="grid size-14 place-items-center rounded-2xl bg-brand-soft text-brand"><UserRound size={24} /></span><h3 className="mt-5 text-lg font-medium">{patients.length === 0 ? "Nenhum paciente cadastrado" : "Nenhum resultado encontrado"}</h3><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">{patients.length === 0 ? "Comece adicionando o primeiro paciente da sua clínica." : "Tente buscar por outro nome."}</p>{patients.length === 0 ? <button className="mt-6 inline-flex h-10 items-center gap-2 rounded-xl bg-brand px-4 text-sm font-medium text-white hover:bg-brand-hover" onClick={() => setIsModalOpen(true)}><Plus size={16} />Adicionar paciente</button> : null}</div> : <div className="divide-y divide-border">{filteredPatients.map((patient) => <article className="flex flex-col gap-4 px-5 py-5 transition hover:bg-surface-muted/50 sm:flex-row sm:items-center sm:px-6" key={patient.id}><span className="grid size-11 shrink-0 place-items-center rounded-full bg-[#f0e4dc] text-sm font-medium text-[#74513c]">{initials(patient.display_name)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{patient.display_name}</p><p className="mt-1 text-xs text-muted-foreground">Prontuário #{patient.record_number} · Nascimento: {formatDate(patient.birth_date)}</p></div><div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><Phone size={13} />{patient.phone_e164 ?? "Telefone não informado"}</span>{patient.email ? <span className="inline-flex items-center gap-1.5"><Mail size={13} />{patient.email}</span> : null}</div></article>)}</div>}
        </section>
      </div>

      {isModalOpen ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-foreground/30 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsModalOpen(false); }}><section aria-label="Novo paciente" className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl border border-border bg-surface p-6 shadow-2xl sm:max-w-xl sm:rounded-3xl sm:p-8"><div className="flex items-start justify-between"><div><p className="text-sm font-medium text-brand">Cadastro</p><h2 className="mt-1 text-2xl font-semibold tracking-[-0.04em]">Novo paciente</h2></div><button aria-label="Fechar cadastro" className="grid size-9 place-items-center rounded-xl text-muted-foreground hover:bg-surface-muted" onClick={() => setIsModalOpen(false)}><X size={18} /></button></div><form className="mt-7 space-y-4" onSubmit={handleSubmit}><label className="block"><span className="mb-2 block text-sm font-medium">Nome de exibição</span><input className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none focus:border-brand" placeholder="Mariana Costa" value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} required /></label><label className="block"><span className="mb-2 block text-sm font-medium">Nome completo / legal</span><input className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none focus:border-brand" placeholder="Nome completo do paciente" value={form.legalName} onChange={(event) => setForm({ ...form, legalName: event.target.value })} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-sm font-medium">Data de nascimento</span><span className="relative block"><CalendarDays className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input className="h-11 w-full rounded-xl border border-border bg-background pl-10 pr-4 text-sm outline-none focus:border-brand" type="date" value={form.birthDate} onChange={(event) => setForm({ ...form, birthDate: event.target.value })} /></span></label><label className="block"><span className="mb-2 block text-sm font-medium">Telefone</span><input className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none focus:border-brand" placeholder="(11) 99999-0000" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label></div><label className="block"><span className="mb-2 block text-sm font-medium">E-mail</span><input className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none focus:border-brand" type="email" placeholder="paciente@email.com" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label className="block"><span className="mb-2 block text-sm font-medium">Observações internas</span><textarea className="min-h-24 w-full resize-y rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-brand" placeholder="Informações administrativas não clínicas" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label><div className="flex flex-col-reverse gap-3 pt-3 sm:flex-row sm:justify-end"><button className="h-11 rounded-xl px-4 text-sm font-medium text-muted-foreground hover:bg-surface-muted" type="button" onClick={() => setIsModalOpen(false)}>Cancelar</button><button className="h-11 rounded-xl bg-brand px-5 text-sm font-medium text-white hover:bg-brand-hover disabled:opacity-60" disabled={isSaving} type="submit">{isSaving ? "Salvando..." : "Salvar paciente"}</button></div></form></section></div> : null}
    </main>
  );
}
