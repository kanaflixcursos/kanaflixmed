...
type AppointmentAction = { status: string; label: string; icon: typeof Check };
...
export default function AgendaPage() {
  ...
  const grouped = useMemo(() => appointments.reduce<Record<string, Appointment[]>>((groups, appointment) => {
    const key = time(appointment.startsAt);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!service || !form.patientId || !form.startsAt) {
      setError("Selecione paciente, serviço e horário.");
      setIsSaving(false);
      return;
    }
    ...
    try {
      const response = await fetch("/api/appointments", {
        ...
        body: JSON.stringify({ patientId: form.patientId, serviceId: form.serviceId, startsAt: new Date(form.startsAt).toISOString(), endsAt: endFromStart(form.startsAt, service.duration_minutes), operationalNote: form.notes }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error === "AUTH_REQUIRED" ? "Entre na sua conta antes de agendar." : payload.error === "PATIENT_NOT_FOUND" ? "Esse paciente não está mais ativo." : "Revise os dados do agendamento.");
      setForm(emptyForm);
  ...
  function actionFor(status: string): AppointmentAction | null {
    if (status === "SCHEDULED") return { status: "CONFIRMED", label: "Confirmar", icon: Check };
    if (status === "CONFIRMED") return { status: "CHECKED_IN", label: "Fazer check-in", icon: CheckCircle2 };
    if (status === "CHECKED_IN") return { status: "IN_PROGRESS", label: "Iniciar atendimento", icon: Play };
    if (status === "IN_PROGRESS") return { status: "COMPLETED", label: "Concluir", icon: CheckCircle2 };
    return null;
  }

  async function handleStatusChange(appointment: Appointment, nextStatus: string) {
    setUpdatingAppointmentId(appointment.id);
    setError(null);
    try {
      const response = await fetch("/api/appointments", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: appointment.id, status: nextStatus }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error === "INVALID_STATUS_TRANSITION" ? "Essa mudança de status não está disponível." : payload.error === "APPOINTMENT_CONFLICT" ? "O agendamento foi atualizado por outra pessoa. Recarregue a agenda." : "Não foi possível atualizar o agendamento.");
      await loadData();
    } catch (statusError) {
      setError(statusError instanceof Error ? statusError.message : "Não foi possível atualizar o agendamento.");
    } finally {
