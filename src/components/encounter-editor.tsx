...
export function EncounterEditor({ id }: { id: string }) {
  ...
  const readOnly = encounter?.status === "FINALIZED" || !canWrite;
  return <main className="min-h-screen bg-background px-5 py-8 sm:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-4xl">
    ...
    {!readOnly && appointmentStatus !== "COMPLETED" ? <p className="mt-5 rounded-xl border border-warning/20 bg-warning/5 px-4 py-3 text-sm text-warning">Você pode salvar o rascunho agora. O encerramento fica disponível depois que a consulta for concluída.</p> : null}
    {readOnly ? <p className="mt-5 inline-flex items-center gap-2 rounded-xl bg-background px-4 py-3 text-sm text-muted-foreground"><LockKeyhole size={16} />{encounter?.status === "FINALIZED" ? `Prontuário encerrado em ${new Intl.DateTimeFormat("pt-BR", { dateStyle: "long", timeStyle: "short" }).format(new Date(encounter.finalized_at!))}. Somente leitura.` : "Acesso somente para leitura."}</p> : null}
    <form onSubmit={saveDraft} className="mt-6 space-y-4">{fields.map((field) => <label key={field.key} className="block rounded-2xl border border-border bg-surface p-4 sm:p-5"><span className="mb-2 block text-sm font-medium">{field.label}</span><textarea value={form[field.key]} onChange={(event) => setForm((previous) => ({ ...previous, [field.key]: event.target.value }))} disabled={readOnly} maxLength={12000} placeholder={field.placeholder} className="min-h-28 w-full resize-y rounded-xl border border-border bg-background px-4 py-3 text-sm leading-6 disabled:opacity-80" /></label>)}
      {!readOnly ? <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end"><button disabled={saving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border px-4 text-sm font-medium hover:border-brand disabled:opacity-60"><Save size={16} />{saving ? "Salvando…" : "Salvar rascunho"}</button>{appointmentStatus === "COMPLETED" ? <button type="button" disabled={saving} onClick={() => void persist(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-medium text-white hover:bg-brand-hover disabled:opacity-60"><FileCheck2 size={16} />Encerrar prontuário</button> : null}</div> : null}
    </form>
  </div></main>;
}
