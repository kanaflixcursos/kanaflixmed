...
export function AppointmentDetail({ id, returnTo }: { id: string; returnTo: string }) {
  ...
  const nextAction: Record<string, { status: string; label: string; icon: typeof Check }> = {
    ...
    CONFIRMED: { status: "IN_PROGRESS", label: "Iniciar atendimento / check-in", icon: CheckCircle2 },
  ...
  const encounterActionLabel = !["IN_PROGRESS", "COMPLETED", "CHECKED_IN"].includes(appointment.status) ? null
