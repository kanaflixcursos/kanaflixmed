...
import { ArrowLeft, CalendarDays, Check, Clock3, UserRound } from "lucide-react";
...
export function NewAppointmentForm({ initialDate, initialTime, returnTo }: { initialDate: string; initialTime: string; returnTo: string }) {
  ...
  return <main className="min-h-screen bg-background px-5 py-8 sm:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-4xl">
    ...
    {loading ? <div className="mt-6 rounded-3xl border border-border bg-surface p-10 text-center text-sm text-muted-foreground">Carregando informações…</div> : patients.length === 0 ? <section className="mt-6 rounded-3xl border border-border bg-surface p-8 text-center"><UserRound className="mx-auto text-brand" /><h2 className="mt-3 font-medium">Cadastre um paciente primeiro</h2><p className="mt-1 text-sm text-muted-foreground">O agendamento precisa estar ligado a um paciente ativo.</p><Link href="/patients" className="mt-5 inline-flex h-10 items-center rounded-xl bg-brand px-4 text-sm font-medium text-white">Abrir pacientes</Link></section> : <form onSubmit={submit} className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]"><section className="space-y-5 rounded-3xl border border-border bg-surface p-5 shadow-sm sm:p-7"><div><h2 className="text-lg font-medium">Dados do atendimento</h2><p className="mt-1 text-sm text-muted-foreground">As informações ficam vinculadas ao cadastro do paciente.</p></div>
      <label className="block"><span className="mb-2 block text-sm font-medium">Paciente</span><select required value={patientId} onChange={(event) => setPatientId(event.target.value)} className="h-12 w-full rounded-xl border border-border bg-background px-3 text-sm"><option value="">Selecione um paciente</option>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.display_name}</option>)}</select></label>
      ...
      <button type="submit" disabled={saving || !serviceId || !professionalId} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-medium text-white hover:bg-brand-hover disabled:opacity-60"><Check size={17} />{saving ? "Salvando…" : "Salvar agendamento"}</button>
    </section></form>}
  </div></main>;
}
